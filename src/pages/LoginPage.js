...
const LoginPage = ({ navigate }) => {
    ...
    // Fetch free tests on component mount to display them as a preview
    useEffect(() => {
        const fetchFreeTests = async () => {
            ...
            try {
                ...
                for (const section of SECTIONS) {
                    const q = query(
                        collection(db, 'tests'), 
                        where("type", "==", "SECTIONAL"), 
                        where("sections.0.name", "==", section),
                        where("isFree", "==", true),
                    ...
                    if (!snapshot.empty) {
                        ...
                        availableTests.push({ id: testDoc.id, ...testDoc.data() });
                    }
                }
                setFreeTests(availableTests);
            } catch (error) {
    ...
    const features = [
        {
            ...
            description: "Experience the actual exam environment to build confidence and master time management before the big day."
        },
        {
            icon: <svg className="w-8 h-8 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"></path></svg>,
            title: "In-Depth Analytics",
            description: "Go beyond just a score. Understand your strengths and pinpoint your weaknesses with our detailed post-test analysis."
        },
        {
            ...
            description: "Attempt full-length and sectional tests designed by experts to perfectly match the latest CAT pattern and difficulty."
        }
    ];
                ...
                <section className="py-20 bg-gray-900">
                    <div className="container mx-auto px-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                            {loading ? <p className="text-center col-span-3 text-gray-400">Loading free tests...</p> : freeTests.map(test => (
                                <div key={test.id} className="bg-gray-800 rounded-lg shadow-lg p-6 flex flex-col justify-between">
                                    <div>
                        ...
                        <div className="text-center mt-12">
                             <button onClick={() => navigate('subscription')} className="bg-amber-500 text-gray-900 px-8 py-3 rounded-md font-semibold hover:bg-amber-400 shadow transition-all transform hover:scale-105">
                                ...
                                <div key={index} className="lg:pt-12 pt-6 w-full md:w-4/12 px-4 text-center">
                                    <div className="relative flex flex-col min-w-0 break-words bg-gray-900 w-full mb-8 shadow-lg rounded-lg">
                                        <div className="px-4 py-5 flex-auto">
                                            <div className="text-white p-3 text-center inline-flex items-center justify-center w-16 h-16 mb-5 shadow-lg rounded-full bg-gray-700">
                                                {feature.icon}
                                            </div>
                                            <h6 className="text-xl font-semibold text-white">{feature.title}</h6>
                                            <p className="mt-2 mb-4 text-gray-400">{feature.description}</p>
                                        </div>
                                    </div>
                                </div>
                            ))}
